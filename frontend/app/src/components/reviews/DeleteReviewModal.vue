...
export default {
  ...
  watch: {
    show(newVal) {
      if (newVal) {
        this.openModal('delete-review-modal')
      } else {
        this.closeModal('delete-review-modal')
      }
    }
  },
  methods: {
    async deleteItem() {
      ...
      try {
        ...
        const response = await axios.delete(`/user/business/review/${itemId}`)

        if (response.data.status) {
          this.$emit('item-deleted', this.item, response.data.message || 'Item deleted successfully')
          this.$emit('close')
        }
      } catch (error) {
