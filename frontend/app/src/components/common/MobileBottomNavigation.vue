  ...
  <div>
    ...
    >
      ...
      >
        ...
        <span
          class="text-xs font-medium text-center leading-tight transition-all duration-300"
          :class="{
            'font-semibold text-primary': $route.name === 'home',
            'text-gray-600': $route.name !== 'home',
          }"
          >Home</span
        >
      </div>
      ...
      <div
        class="flex flex-col items-center cursor-pointer px-3 py-2 rounded-2xl transition-all duration-300 ease-out min-w-12"
        :class="{
          'text-primary -translate-y-0.5': $route.path === '/random/products',
          'text-gray-600 hover:-translate-y-0.5':
            $route.path !== '/random/products',
        }"
        @click="navigateTo('random-products')"
      >
        <div class="flex items-center justify-center mb-1.5">
          <ArrowPathRoundedSquareIcon
            v-if="$route.path === '/random/products'"
            class="w-6 h-6 transition-all duration-300"
            :class="{ 'scale-110': $route.path === '/random/products' }"
          />
        ...
        <span
          class="text-xs font-medium text-center leading-tight transition-all duration-300"
          :class="{
            'font-semibold text-primary': $route.path === '/random/products',
            'text-gray-600': $route.path !== '/random/products',
          }"
          >Random</span
        >
      </div>
      ...
      >
        <div class="flex items-center justify-center mb-1.5 relative">
          ...
          />
          <div
            v-if="isAuthenticated && hasUnreadMessages"
        ...
        <span
          class="text-xs font-medium text-center leading-tight transition-all duration-300"
          :class="{
            'font-semibold text-primary': $route.name === 'conversations',
            'text-gray-600': $route.name !== 'conversations',
          }"
          >Message</span
        >
      </div>
      ...
      >
        ...
        <span
          class="text-xs font-medium text-center leading-tight transition-all duration-300"
          :class="{
            'font-semibold text-primary': isUserRoute,
            'text-gray-600': !isUserRoute,
          }"
          >Profile</span
        >
      </div>
...
export default {
  ...
  computed: {
    ...
    isUserRoute() {
      ...
      return (
        (userRoutes.includes(this.$route.name) ||
  ...
  methods: {
    navigateTo(routeName) {
      ...
      } else if (routeName === "random-products") {
        // Navigate to random products page
        this.$router.push({ name: routeName });
