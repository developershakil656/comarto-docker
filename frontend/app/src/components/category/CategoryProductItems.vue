    ...
    <div v-else class="space-y-6">
      <!-- Total products found -->
        <div class="md:flex justify-between items-center w-full">
          <div>
            <h1 v-if="categoryName">{{ categoryName }} {{ isSuppliersView ? "suppliers" : "products" }} in
            {{ this.$store.getters.selectedLocation || 'All Bangladesh' }}</h1>
            <h1 v-else>Products in {{ categorySlug }}</h1>
