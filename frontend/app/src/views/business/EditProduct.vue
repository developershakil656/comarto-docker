<template>
  <div>
    <AdminDashboardHeader title="Edit Product" backUrl="/my/business/products"/>
    <div class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-primary-100 py-8">
      <div class="max-w-4xl mx-auto px-4">
        ...
        <div class="text-center mb-6 md:mb-8">
          ...
          <p class="text-gray-600 mt-2">Update and manage your product information</p>
        </div>
        ...
        <div v-else>
          ...
          <div v-if="currentStep === 0" class="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-6 border border-primary-100">
            ...
            <form @submit.prevent="nextStep" class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label for="edit-product-name" class="block text-sm font-semibold text-gray-700 mb-2">Product Name *</label>
                  <input
                    v-model="form.name"
                    type="text"
                ...
                <div>
                  <label for="edit-product-stock" class="block text-sm font-semibold text-gray-700 mb-2">Stock Status *</label>
                  <select
                    v-model="form.stock"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div class="flex items-center space-x-2">
                    <span class="text-primary-600 font-medium">Per</span>
                    <input
                      v-model="form.unit_quantity"
                      type="number"
                      min="1"
                      class="w-20 px-3 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-300 text-center hover:border-primary-200"
                      placeholder="1"
                      required
                    />
                    <div class="relative flex-1">
                         ...
                         required
                       />
                                             <div v-if="unitDropdown.isOpen" class="absolute z-50 w-full mt-1 bg-white border-2 border-primary-200 rounded-xl shadow-xl max-h-60 overflow-y-auto" style="top: 100%; left: 0;">
                         <div v-if="unitsLoading" class="px-4 py-3 text-gray-500 text-center">
                           Loading units...
                         </div>
                         <div v-else-if="filteredUnits.length === 0" class="px-4 py-4 text-gray-500 text-center">
                           No units found
                         </div>
                         <div
                           v-else
            ...
            <div class="space-y-6">
              ...
              <div class="text-sm text-gray-600 mb-2">
                ...
                <div class="relative">
                   ...
                   />
                                     <div v-if="categoryDropdown.isOpen" class="absolute z-50 w-full mt-1 bg-white border-2 border-primary-200 rounded-xl shadow-xl max-h-60 overflow-y-auto" style="top: 100%; left: 0;">
            ...
            <div class="space-y-6">
              ...
              <div>
                ...
                <div class="border-2 border-dashed border-primary-200 rounded-xl p-6 text-center hover:border-primary-300 transition-colors duration-300">
                  ...
                  />
                  <label for="image-upload" class="cursor-pointer" :class="{ 'opacity-50 cursor-not-allowed': imageUploading }">
                    ...
                    <p class="mt-2 text-sm text-primary-600">
                      ...
                      <span v-else>Click to upload images</span>
                    </p>
              ...
              <div v-if="images.length > 0">
                <div class="flex items-center justify-between mb-2">
                  ...
                  <div v-if="imagePositionChanging" class="flex items-center text-sm text-primary-600">
                    <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-primary mr-2"></div>
                    Updating order...
                  </div>
                </div>
                <p class="text-sm text-gray-500 mb-4">Drag and drop images to reorder. The first image will be the feature image.</p>
                <div class="relative">
                <draggable
                  v-model="images"
                  ...
                  @cancel="onDragCancel"
                  item-key="index"
                  :disabled="imagePositionChanging"
                >
                  <template #item="{ element: image, index }">
                    <div class="relative group aspect-square">
                      <img
                        ...
                        class="w-full h-full object-cover rounded-xl border-4 border-gray-300 transition-all duration-300"
                        :class="{ 
                          'border-primary': index === 0,
                          'border-blue-400': drag && dragStartIndex === index
                        }"
                      />
                  ...
                  <!-- Loading overlay -->
                  <div v-if="imageUploading" class="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center rounded-xl">
                    <div class="text-center">
                      <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
                      <p class="text-primary-600 font-medium">Uploading images...</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-if="images.length === 0" class="text-center py-8 text-gray-500">
                <p v-if="imageUploading">Uploading images...</p>
                <p v-else>No images uploaded yet. Upload images above to get started.</p>
              </div>
              
              <div v-if="fieldErrors.images" class="mt-1 text-sm text-red-600">
                {{ fieldErrors.images }}
              </div>
            </div>
          </div>
          ...
          <div class="flex justify-between items-center bg-white rounded-2xl shadow-xl p-4 md:p-6 border border-primary-100 text-sm md:text-base">
            <button
              @click="prevStep"
              v-if="currentStep > 0"
              class="px-4 md:px-6 py-3 border-2 border-primary-200 text-primary-700 rounded-xl hover:border-primary-300 hover:bg-primary-50 transition-all duration-300"
            >
              ← Previous
            </button>
            <div v-else></div>
            ...
            <div class="flex space-x-4">
              <button 
                v-if="currentStep < 3"
                @click="nextStep"
                class="px-6 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-xl hover:from-primary-600 hover:to-primary-700 transition-all duration-300 shadow-lg hover:shadow-xl"
              >
                Next →
              </button>
              <button
...
export default {
  ...
  computed: {
    ...
    isSubmitting() {
      return this.isSubmitting; // Use the data property
    },
  ...
  async mounted() {
    ...
    // Don't trigger initial searches - let them happen on user interaction
  ...
  methods: {
    ...
    async fetchProduct() {
      try {
        ...
        const response = await this.$store.dispatch('fetchProduct', productId);
    ...
    populateForm(product) {
      // Determine price type and set values
      let priceType = 'single';
      let singlePrice = '';
      let priceRange = { min: '', max: '' };
      
      if (product.price) {
        // Check if price contains a range indicator (e.g., "90 - 100" or "90-100")
        if (product.price.includes('-') || product.price.includes('–')) {
          priceType = 'range';
          const rangeParts = product.price.split(/[-–]/).map(part => part.trim());
          if (rangeParts.length === 2) {
            priceRange.min = parseFloat(rangeParts[0]) || '';
            priceRange.max = parseFloat(rangeParts[1]) || '';
          }
        } else {
          // Single price
          singlePrice = parseFloat(product.price) || '';
        }
      }
      ...
      this.form = {
        ...
        product_unit_id: product.product_unit?.id || null,
        stock: product.stock || '',
      ...
      // Convert specification object to array
      this.specifications = Object.entries(product.specification || {}).map(([key, value]) => ({
        key,
        value
      }));
      ...
      // Set images - ensure they have the correct structure
      this.images = (product.gallery || []).map(img => ({
        id: img.id,
        url: img.url || img.image_url || img,
        isNew: false
      }));
      
      // Set unit dropdown search
      if (product.product_unit) {
        this.unitDropdown.search = product.product_unit.full_form || product.product_unit.short_form;
      }

      // Set video preview if available
      if (product.video_url) {
        this.fetchVideoPreview(product.video_url);
      }
    },
    nextStep() {
    ...
    handleUnitFocus() {
      ...
      // Always trigger search on focus to ensure fresh data
      this.searchUnits();
    },
    handleCategoryFocus() {
      // Always open dropdown on focus to allow searching
    ...
    async onImageChange(event) {
      ...
      try {
        ...
        console.log('Starting to upload', files.length, 'images');
        
        // Upload each new image individually
        ...
        push.success('Images uploaded successfully!');
        ...
        push.error(error.message || 'Failed to upload images. Please try again.');
      } finally {
    ...
    async uploadImage(file) {
      ...
      try {
        ...
        // Call the store action to upload the image as File object
        const response = await this.$store.dispatch('uploadProductImages', {
          productId,
          images: [file]
        });
        ...
        if (response) {
          ...
          console.log('Image added to local array, total images:', this.images.length);
        }
        return response;
      } catch (error) {
    ...
    async removeImage(index) {
      ...
      // If it's a new image (not yet uploaded), just remove from array
      if (image.isNew) {
      ...
      if (image.id) {
        try {
          ...
          push.error(error.message || 'Failed to delete image. Please try again.');
        } finally {
    ...
    async deleteImage(imageId) {
      try {
        const response = await axios.delete(`/user/product/gallery/${imageId}`, {
          headers: {
    ...
    async onImageReorder(event) {
      ...
      // Only proceed if we have images with IDs
      ...
      try {
        ...
        // Create payload with all image IDs in their current positions
        const imageIds = this.images.map((image, index) => ({
          id: image.id,
          position: index + 1
        })).filter(img => img.id); // Only include images with IDs
        
        // Update positions in the backend
        await this.updateProductImagePositions({
          productId: this.$route.params.id,
          imageIds: imageIds
        });
        
        push.success('Image order updated successfully!');
        ...
        push.error(error.message || 'Failed to update image positions. Please try again.');
      } finally {
    ...
    async updateImagePositions(imageIds) {
      try {
        ...
        const response = await axios.put('/user/product/gallery/positions', payload, {
          headers: {
            'Authorization': `Bearer ${this.$store.state.token}`,
            'Content-Type': 'application/json'
          }
    ...
    async onSubmit() {
      ...
      try {
        ...
        if (productResult && productResult.id) {
          ...
          // Redirect to business products page after successful product update
          setTimeout(() => {
        ...
        push.error(error.message || 'Failed to update product. Please try again.');
      } finally {
        this.loading = false;
      }
    },
    ...
    handleClickOutside(event) {
      // Check if click is outside the unit dropdown container
      const unitContainer = event.target.closest('.relative');
      if (!unitContainer) {
        this.unitDropdown.isOpen = false;
      }
      
      // Check if click is outside the category dropdown container
      const categoryContainer = event.target.closest('.relative');
    ...
    // Method to fetch video preview
    async fetchVideoPreview(videoUrl) {
      try {
        this.videoPreview = null; // Clear previous preview
    ...
    // Helper to format YouTube duration string (e.g., PT1M30S -> 1:30)
    ...
    // Helper to validate if a URL is a YouTube URL
