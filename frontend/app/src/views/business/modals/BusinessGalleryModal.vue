<template>
  ...
  >
    <div
      class="bg-white w-full h-full md:h-full md:rounded-lg md:shadow-xl md:max-w-4xl overflow-y-auto"
    >
        ...
        <div class="bg-white rounded-2xl shadow-lg p-4 py-6 min-h-full">
          ...
          <div v-else class="space-y-6">
            ...
            <div class="relative group">
              <span
                class="absolute left-4 top-6 -translate-y-1/2 text-gray-400 pointer-events-none"
              >
                <svg
                  class="w-5 h-5"
                  fill="none"
              ...
              <label
                ...
                :class="{
                  'top-0 text-xs left-8 text-primary':
                    form.video_url && !fieldErrors.video_url,
              ...
              <!-- Error Message -->
              <div
                v-if="fieldErrors.video_url"
                class="mt-1 text-red-500 text-sm flex items-center"
              >
                <svg
                  class="w-4 h-4 mr-1"
                  fill="none"
            ...
            <div v-if="videoPreview && !fieldErrors.video_url" class="mb-4">
              ...
              <div
                class="border border-gray-200 rounded-lg overflow-hidden bg-gray-50 max-w-xl mx-auto"
              >
            ...
            <div>
              ...
              >
              <div
                class="border-2 border-dashed border-primary-200 rounded-xl p-6 text-center hover:border-primary-300 transition-colors duration-300"
              >
                <input
                ...
                <label for="business-image-upload" class="cursor-pointer">
                  ...
                  <p class="mt-1 text-xs text-gray-500">
                    JPEG, JPG, PNG, and WEBP up to 5MB
                  </p>
                </label>
              </div>
              ...
              >
                <template #item="{ element: image, index }">
                  <div class="relative group">
                    ...
                    <div
                      v-if="image.isNew"
                      class="absolute bottom-2 left-2 bg-yellow-500 text-white text-xs px-2 py-1 rounded-full"
                    >
                      New
                    </div>
                  </div>
            ...
            <div
              v-if="initialImages && initialImages.length > 0"
            ...
            >
              ...
              <p class="text-sm text-blue-700">
                You have {{ initialImages.length }} existing image(s) in your
                business gallery. Upload new images above to add to your
                collection.
              </p>
            </div>
    ...
    <div
      v-if="successMessage"
      class="fixed top-4 right-4 z-50 p-4 bg-green-100 border border-green-400 text-green-700 rounded-xl"
    >
      {{ successMessage }}
    </div>
    <div
      v-if="errorMessage"
      class="fixed top-4 right-4 z-50 p-4 bg-red-100 border border-red-400 text-red-700 rounded-xl"
    >
      {{ errorMessage }}
    </div>
  </div>
  ...
  methods: {
    ...
    async saveGallery() {
      ...
      try {
        ...
        push.error(
          error.message || "Failed to save business gallery. Please try again."
        );
      } finally {
        this.isSaving = false;
      }
    ...
    async onImageReorder(event) {
      ...
      try {
        ...
        this.errorMessage =
          error.message ||
          "Failed to update image positions. Please try again.";
      } finally {
    ...
    async updateImagePositions(imageIds) {
      try {
        ...
        const response = await axios.put(
          "/user/business/gallery/positions",
          payload,
          {
            headers: {
              Authorization: `Bearer ${this.$store.state.token}`,
