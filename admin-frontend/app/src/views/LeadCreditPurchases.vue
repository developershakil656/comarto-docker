<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center admin-header">
      <h1 class="text-2xl font-bold text-gray-900">Lead Credit Purchases</h1>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 responsive-table">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="purchase in purchases" :key="purchase.id">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" data-label="ID">{{ purchase.id || 'N/A' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" data-label="Business">
                <div>{{ purchase.business_name || 'N/A' }}</div>
                <div class="text-xs text-gray-400">ID: {{ purchase.business_id || 'N/A' }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" data-label="Package">{{ purchase.package_name || 'N/A' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" data-label="Credits">{{ purchase.credits || 'N/A' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" data-label="Amount">{{ purchase.amount || 'N/A' }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500" data-label="Payment Method">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium" data-label="Actions">
                <select
                  ...
                  @change="updatePurchaseStatus(purchase.id, purchase.status)"
...
import { useLeadCreditPurchaseStore } from '../stores/leadCreditPurchase'
import { useToast } from '../composables/useToast'

const leadCreditPurchaseStore = useLeadCreditPurchaseStore()
const { showToast } = useToast()

const purchases = computed(() => leadCreditPurchaseStore.purchases)
const loading = computed(() => leadCreditPurchaseStore.loading)
const pagination = computed(() => leadCreditPurchaseStore.pagination)
const error = computed(() => leadCreditPurchaseStore.error)
...
const fetchPurchases = async (params = {}) => {
  try {
    await leadCreditPurchaseStore.fetchAll(params)
  } catch (err) {
    showToast(error.value || 'Failed to fetch purchases', 'error')
  }
}

const updatePurchaseStatus = async (id, status) => {
  try {
    await leadCreditPurchaseStore.updateStatus(id, status)
    showToast('Purchase status updated successfully', 'success')
  } catch (err) {
    showToast(error.value || 'Failed to update purchase status', 'error')
    // Revert the status change in UI
    const purchase = purchases.value.find(p => p.id === id)
    if (purchase) {
      purchase.status = status // This would need to be reverted to the original status
    }
  }
}
