  ...
  <div class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-primary-100 py-8">
    <div class="max-w-4xl mx-auto px-4">
      ...
      <div class="text-center mb-6 md:mb-8">
        ...
        <p class="text-gray-600 mt-2">Update and manage product information</p>
      </div>
      ...
      <div v-else>
        ...
        <div v-if="currentStep === 3" class="bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-6 border border-primary-100">
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 mt-2">First image will be the feature image.</p>
            ...
            <div v-if="imagePositionChanging" class="flex items-center text-sm text-primary-600 mb-2">
        ...
        <div class="flex justify-between items-center bg-white rounded-2xl shadow-xl p-4 md:p-6 border border-primary-100 text-sm md:text-base">
          ...
          <div class="flex space-x-4">
            ...
            >
              <span v-if="isSubmitting">Updating Product...</span>
...
export default {
  ...
  setup() {
    ...
    const imagePositionChanging = ref(false);
    ...
    const fetchProduct = async () => {
      try {
        ...
        push.error('Failed to load product');
      } finally {
        loading.value = false;
      }
    };
    ...
    const populateForm = (product) => {
      ...
      form.value.details = product.details;
      form.value.stock = product.stock;
      form.value.unit_quantity = product.unit_quantity;
      form.value.product_unit_id = product.product_unit.id;
      form.value.video_url = product.video_url || '';
      form.value.status = product.status || 'active';
      form.value.business_id = product.business.id;

      // Handle price
      if (product.price && product.price.includes('-')) {
        priceType.value = 'range';
        const parts = product.price.split('-').map(p => parseFloat(p.trim()));
        form.value.priceRange.min = parts[0];
        form.value.priceRange.max = parts[1];
      } else {
        priceType.value = 'single';
        form.value.singlePrice = parseFloat(product.price) || 0;
      }

      // Handle specifications
      if (product.specification && typeof product.specification === 'object') {
        specifications.value = Object.keys(product.specification).map(key => ({
          key,
      ...
      if (product.categories && Array.isArray(product.categories)) {
        ...
        form.value.category_ids = product.categories.map(c => c.id);
      }

      // Handle images
      if (product.gallery && Array.isArray(product.gallery)) {
        images.value = product.gallery.map(img => ({
          url: img.url,
          id: img.id
        }));
      }
      ...
      if (product.product_unit && product.product_unit.full_form) {
        unitDropdown.value.search = product.product_unit.full_form;
      }
    };
    ...
    const onImageChange = async (event) => {
      ...
      files.forEach(file => {
        ...
        // build object for upload; don't add to the main images list yet
        const imageObj = { file, preview, isNew: true };
        newUploads.push(imageObj);
      });

      // if editing an existing product, upload immediately
      if (productId.value && newUploads.length > 0) {
        try {
          const formData = new FormData();
          newUploads.forEach((img) => {
            if (img.file) formData.append('images[]', img.file);
          });
          const response = await productAPI.uploadImages(productId.value, formData);
          // response may contain uploaded image data; assign ids/urls and clear isNew flags
          const uploaded = response.data?.data?.images || [];
          // Map returned server images back to our local objects and update the main images array
          uploaded.forEach((serverImg, idx) => {
            const localImg = newUploads[idx];
            if (localImg) {
              // build the final image object to store in images list
              const finalImg = {
                id: serverImg.id,
                url: serverImg.url,
              ...
              images.value.push(finalImg);
            }
          });
          push.success('Image(s) uploaded successfully');
        } catch (err) {
          console.error('Immediate image upload failed:', err);
          push.error('Failed to upload one or more images. Please try again.');
        }
      }
    };
    ...
    const removeImage = async (index) => {
      ...
      // if the image has already been persisted on the server, call delete endpoint
      if (img && img.id && !img.isNew) {
        try {
          await productAPI.deleteImage(img.id);
          push.success('Image removed successfully');
        } catch (err) {
          console.error('Failed to delete image:', err);
          push.error('Failed to remove image from server');
          // still remove from UI so user can continue, but they may need to retry later
        }
      }
      images.value.splice(index, 1);
    };

    const onImageReorder = async () => {
      if (!productId.value) return;
      try {
        imagePositionChanging.value = true;
        const orderPayload = images.value
          .map((img, idx) => ({ id: img.id, position: idx + 1 }))
          .filter(o => o.id);
        if (orderPayload.length === 0) return;
        await productAPI.reorderImages(productId.value, { image_ids: orderPayload });
        push.success('Image order updated successfully');
      } catch (err) {
        console.error('Failed to reorder images:', err);
        push.error('Failed to update image order');
      } finally {
        imagePositionChanging.value = false;
      }
    };
    ...
    const validateStep4 = () => {
      ...
      if (images.value.length === 0) {
        fieldErrors.value.images = 'Please have at least one image';
        return false;
      }
      return true;
    };
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        push.error(error.response?.data?.message || 'Failed to update product. Please try again.');
      } finally {
